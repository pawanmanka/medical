                ...
                <div class="ibox ">
                    ...
                    <div class="ibox-content">
                        ...
                        <form method="post" id='user_form'>
                            ...
                            <div class="form-group  row"><label class="col-sm-2 col-form-label">Select Permissions</label>
                                <div class="col-sm-10">
                                    @foreach (config('application.rolePermissionArr') as $key => $item)
                                       {{ ucfirst($key) }} 
                                       <br> 
                                       @foreach ($item as $index => $each)
                                        <?php $value ="$index $key"; ?>
                                         @if(isset($permissions[$value]))
                                         <input type="checkbox" checked name="permission[{{$key}}][{{$index}}]" id=""> {{ $each }}
                                         @else
                                         <input type="checkbox" name="permission[{{$key}}][{{$index}}]" id=""> {{ $each }}
                                         @endif 
                                        @endforeach
                                        <br>
                                    @endforeach
